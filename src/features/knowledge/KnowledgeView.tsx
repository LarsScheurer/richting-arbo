...
interface KnowledgeViewProps {
  ...
  // Passing knowledge structure as prop or importing from constants file
  knowledgeStructure: typeof KNOWLEDGE_STRUCTURE;
}

export const KnowledgeView: React.FC<KnowledgeViewProps> = ({ documents, openDocument, knowledgeStructure }) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        ...
        >
          ...
          {knowledgeStructure.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
        </select>
      </div>
